from mlx import Mlx
from .display_utils import themes, controls_set
...
def draw_empty_maze(maze: str, maze_width: int, x: int, y: int) -> None:
    ...
    for cell in maze:
        ...
        image = mlx.mlx_png_file_to_image(connection, themes[theme_index][f"{cell}"])
...
def draw_path(path: str, start_point: list[int]):
    ...
    for index in range(len(path) - 1):  # Skip exit point to not draw over it
        ...
        path_pixel = mlx.mlx_png_file_to_image(connection, themes[theme_index]["path"])
...
def draw_points(entry_coords: list[int], exit_coords: list[int]) -> None:
    ...
    entry_img = mlx.mlx_png_file_to_image(connection, themes[theme_index]["entry"]) # To be edited
    exit_img = mlx.mlx_png_file_to_image(connection, themes[theme_index]["exit"]) # To be edited
...
def display_output(specs: dict[str, list[int] | str | int], first_time: bool, show: bool)-> None:    
    ...
    # Adds a background
    background = mlx.mlx_png_file_to_image(connection, themes[theme_index]["background"])
